...
const DetailsCard: React.FC<DetailsCardProps> = ({ detail, selectedRace, onSave }) => {
  ...
  // Determine if the selected race has already occurred.
  ...
  return (
    <div className="relative flex-1 bg-gray-800 border border-gray-700 p-6 sm:p-8 rounded-xl shadow-lg mt-6">
      ...
      <p className="mb-3 text-gray-300">
        <span className="font-semibold text-white">Part Name:</span> {detail.UpdatedComponent}
      </p>
      <p className="mb-3 text-gray-300">
        <span className="font-semibold text-white">Reason for upgrade:</span> {detail.PrimaryReason}
      </p>
      <p className="mb-3 text-gray-300">
        <span className="font-semibold text-white">Geometric Differences:</span> {detail.GeometricDifferences}
      </p>
      <p className="text-gray-300">
        <span className="font-semibold text-white">Description:</span> {detail.Description}
      </p>

      {/* Conditionally render the Spoiler section if it's a past race */}
